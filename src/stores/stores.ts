...
export const selectedPieceStore: Writable<CoordType | undefined> = writable(undefined);
...
const createBoard = () => {
	...
	const setPieceType = (x: number, y: number, type: CellType['piece']['type'] = 'lady') => {
		const previousPiece = get(boardStore)[x][y].piece;
		updateBox(x, y, { ...previousPiece, type });
	};

	const updateBox = (lineToUpdate: number, colToUpdate: number, piece: CellType['piece']) =>
		update((board) =>
			board.map((line, lineIndex) =>
				lineIndex === lineToUpdate
					? line.map((col, colIndex) =>
							colIndex === colToUpdate && col.background === 'black' ? { ...col, piece } : col
					  )
					: line
			)
		);
